...
class GraphConvolution_att(nn.Module):
    ...
    def __init__(self, in_features, out_features, num_nodes, bias=True, init_A=0):
...
class GC_Block(nn.Module):
    ...
    def __init__(self, in_features, hidden_features, num_nodes, p_dropout, bias=True, is_resi=True):
        super(GC_Block, self).__init__()
        self.in_features = in_features
        self.out_features = hidden_features
        self.is_resi = is_resi

        self.gc1 = GraphConvolution_att(in_features, in_features, num_nodes)
        self.bn1 = nn.BatchNorm1d(num_nodes * in_features)

        self.gc2 = GraphConvolution_att(in_features, in_features,num_nodes)
...
class GCN_muti_att(nn.Module):
    def __init__(self, input_feature, hidden_feature, num_class,num_nodes, p_dropout, num_stage=1, is_resi=True):
        super(GCN_muti_att, self).__init__()
        self.num_stage = num_stage

        self.gc1 = GraphConvolution_att(input_feature, hidden_feature,num_nodes)
        self.bn1 = nn.BatchNorm1d(num_nodes * hidden_feature)

        self.gcbs = []
        for i in range(num_stage):
            if i == 0:
                self.gcbs.append(GC_Block(hidden_feature, hidden_feature,num_nodes, p_dropout=p_dropout, is_resi=is_resi))
            else:
                self.gcbs.append(GC_Block(hidden_feature, hidden_feature, num_nodes, p_dropout=p_dropout, is_resi=is_resi))
...
if __name__ == '__main__':
    ...
    model = GCN_muti_att(input_feature=num_samples*2, hidden_feature=256,
                         num_class=100, num_nodes=NUM_KEYPOINTS, p_dropout=0.3, num_stage=2)
    x = torch.ones([2, NUM_KEYPOINTS, num_samples*2])
    print(model(x).size())
