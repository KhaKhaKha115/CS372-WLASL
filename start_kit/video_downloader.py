...
def download_aslpro(url, dirname, video_id):
    ...
    logging.warning(f'Skipping ASLPro SWF video {video_id} due to connection issues.')
...
def download_nonyt_videos(indexfile, saveto='raw_videos'):
    ...
    for entry in content:
        ...
        for inst in instances:
            if instance_counter >= MAX_VIDEOS_PER_GLOSS:
                break
            ...
            # Check if video Existed before --> don't download again
            saveto_path = os.path.join(saveto, '{}.mp4'.format(video_id))
            if os.path.exists(saveto_path):
                logging.info('{} exists at {}'.format(video_id, saveto_path))
                instance_counter+=1 
                continue
    ...
    for entry in content:
        ...
        for inst in instances:
            ...
            if os.path.exists(os.path.join(saveto, video_url[-11:] + '.mp4')) or os.path.exists(os.path.join(saveto, video_url[-11:] + '.mkv')):
                ...
                instance_counter+=1 #Increase counter cause video is downloaded
                continue
            else:
                ...
                if not rv:
                    ...
                    instance_counter+=1 # Only increase if download successfully
                else:
